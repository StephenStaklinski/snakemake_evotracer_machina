...
rule all:
    input:
        ...
        [expand("{outdir}/beam/{mouse}/{cp}/beam_probability_graph.csv", outdir=outdir, mouse=m, cp=get_elements_from_file(f"{outdir}/cp_split/{m}")) for m in mice],
...
rule postprocessBeam:
    input:
        beamTrees = "{outdir}/beam/{mouse}/{cp}/beam_{mouse}_{cp}.trees",
    output:
        beamProbGraph = "{outdir}/beam/{mouse}/{cp}/beam_probability_graph.csv",
        threshold50plot = "{outdir}/beam/{mouse}/{cp}/beam_50.pdf",
        threshold75plot = "{outdir}/beam/{mouse}/{cp}/beam_75.pdf",
        threshold90plot = "{outdir}/beam/{mouse}/{cp}/beam_90.pdf",
    params:
        stateKey = "location",
        primaryTissue = lambda wildcards: config['primaryTissue'][wildcards.mouse],
        experimentTime = lambda wildcards: config['experimentTime'][wildcards.mouse],
